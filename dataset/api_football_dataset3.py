...
class ApiFootballDataset(Dataset):
    ...
    def form_data_transformer(self, X):
        ...
        """
        X_copy = X.copy()
        X_copy.resize(X_copy.shape[0], int(self.longest_form_string_length) * 2)
        # loop through rows in X
    ...
    def __init__(self, df):
        ...
        self.df[self.features.one_hot_encode_features] = self.df[self.features.one_hot_encode_features].astype(str)
        self.df[self.features.normalize_features] = self.df[self.features.normalize_features].astype(float)
        self.df[self.features.percentage_features] = self.df[self.features.percentage_features].astype(str)
        self.df[self.features.form_features] = self.df[self.features.form_features].astype(str)
        self.df[self.features.scores_features] = self.df[self.features.scores_features].astype(str)

        # For form datas, get the longest strings lengths
        home_form_longest_string = self.df['predictions_0_teams_home_league_form'].str.len().max()
        away_form_longest_string = self.df['predictions_0_teams_away_league_form'].str.len().max()
        self.longest_form_string_length = max(home_form_longest_string, away_form_longest_string)
